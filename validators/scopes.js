...
async function validateScopes(method, version, path, spec) {
    ...
    let documentedScopes = null;

    for (const operation of Object.keys(spec.paths[path])) {
        if (spec.paths[path][operation].security != undefined && spec.paths[path][operation].security !== null) {
            for (const auth of spec.paths[path][operation].security) {
                if (auth.userAuth != undefined && auth.userAuth !== null) {
                    await testScopes(httpClient, method, path, "user", auth.userAuth)
                } else if (auth.applicationAuth != undefined && auth.applicationAuth !== null) {
                    await testScopes(httpClient, method, path, "application", auth.userAuth)
                } else {
                    console.log(`Unsupported security type for ${method} ${path}: ${Object.keys(auth)[0]}`)
                }
            }
        }
    }
